...
def main():
    ...
    main_tab = st.radio(
        "Choose a section:", 
        ["🎯 Analyze Tickets", "🔍 Search & Browse", "📊 Analytics", "📚 Confluence Docs", "🔍 Screen Explorer", "🏗️ Manage Screens"],
        horizontal=True
    )
    st.markdown("---")
...
def analyze_ticket_content(analyzer, ticket_data, storage, auto_store, store_format):
    ...
    try:
        ...
        if auto_store:
            try:
                stored_ticket_id = storage.store_ticket(ticket, result, report, store_format)
            except Exception as e:
...
def display_analysis_results_with_confluence(result, report, stored_ticket_id, storage, confluence_context=None):
    ...
    with tab5:
        ...
        st.download_button(
            ...
            data=report,
            file_name=f"analysis_report_{result.ticket.key}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md",
            mime="text/markdown"
        )
...
def search_and_browse_section(storage):
        ...
        if results:
            ...
            for result in results[:10]:  # Show first 10 results
                with st.expander(f"🎫 {result.get('ticket_key', 'Unknown')} - {result.get('title', 'No title')[:50]}..."):
                    ...
                    if st.button(f"View Full Analysis", key=f"view_{result.get('id')}"):
                        # Load and display full analysis
                        full_ticket = storage.get_ticket(result['id'])
                        if full_ticket:
                            st.json(full_ticket)
        else:
    ...
    recent_tickets = storage.get_recent_tickets(limit=5)
...
def display_knowledge_base_overview(confluence):
    ...
    try:
        ...
        with col2:
            st.metric("Technologies", stats['unique_technologies'])
        with col3:
            st.metric("Components", stats['total_components'])
        with col4:
            st.metric("Features", stats['total_features'])
